...
const style = {
  container: `w-full`,
  wrapper: `bg-[#303339] flex-auto lg:h-[22rem] rounded-2xl overflow-hidden cursor-pointer`,
...
const NFTCard = ({ nftItem, title, listings, collectionId }) => {
  ...
  useEffect(() => {
    ...
    if (nftItem.price != "" && nftItem.price != null) {
      setIsListed(true);
      setPrice(nftItem.price);
      if (nftItem.currency != "" || nftItem.currency != null) { setCurrency(nftItem.currency); }
    }
  ...
  return <div className={style.container}>
    <div
      ...
      onClick={() => {
        Router.push({
          pathname: `/nfts/${nftItem.moduleAddress}`,
          query: { collectionId: collectionId, isListed: isListed },
        })
      }}
    >
